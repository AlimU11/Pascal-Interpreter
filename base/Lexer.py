...
class Lexer(object):
    def __init__(self, text):
        ...
        if len(text) < 1:
            self.error('SyntaxError: Unexpected EOF While Parsing')

        self.current_char = self.text[self.pos]

    def error(self, message=None):
        raise Exception(message if message else 'Invalid character')
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == '/':
                self.advance()
                self.error('Float division is not supported. Use div operator instead.')
